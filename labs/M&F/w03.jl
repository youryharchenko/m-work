...
# ╔═╡ a4ed5d67-e3d8-44ac-b8f6-52c1405b8be0
using CommonMark

# ╔═╡ 7732688e-4549-11ed-04bc-174949221f20
using CSV, Dates, DataFrames, StateSpaceModels, Plots;

# ╔═╡ e3419d6b-7e53-4e79-b9df-1d76bb8625fa
cm"""
---
<div align="center">
...
Побудова прогнозних моделей з використанням методу кривих підгонки та ex-post прогнозування
...
# ╔═╡ b7a2d18a-bd22-4d22-9fe5-3516da6dcd82
cm"""
...
# ╔═╡ c416f634-ee34-41e1-a75a-bba8e196dfff
csv = CSV.File("data.csv"; select=["year", "total"], types=Dict(:year => Date));

# ╔═╡ 2be5d51f-f2cd-468f-9dd3-d67fafa4a3a2
ts = csv[:total];

# ╔═╡ 790d2776-4256-4bba-b12b-676d053b3fba
cm"""
...
# ╔═╡ 63a87571-4a33-470d-8eed-6dcbb51affcf
let
	model = Naive(ts)
	fit!(model)
	#print_results(model)
	forecast_model = forecast(model, 3)
	plot(model, forecast_model)
end

# ╔═╡ a331c71d-4c7e-46cd-a6e7-5f66eaecc768
cm"""
...
# ╔═╡ 3e8e7ccc-1137-4fd5-aa66-380c099834dc
let
	model = LocalLevel(ts)
	fit!(model)
	print_results(model)
...
# ╔═╡ 25b9b3d0-0333-4a0a-9459-949861993d5c
cm"""

#### Auto ETS

Automatically fits the best ExponentialSmoothing model according to the best AIC 
between the models:
 * ETS(A, N, N)
 * ETS(A, A, N)
 * ETS(A, Ad, N)
...
# ╔═╡ ba6921df-7337-4230-99e3-af0fba9924b6
let
	model = auto_ets(ts)
	fit!(model)
	print_results(model)
...
# ╔═╡ 295b5433-21ae-4562-837f-491bd4e70783
cm"""
...
# ╔═╡ 1e3a73a4-b059-4968-98b4-ebfb7162bfcc
let
	model = auto_arima(ts)
	fit!(model)
	print_results(model)
...
# ╔═╡ d69aa4e4-b7c7-4856-a673-7482681a8a15
cm"""

### Висновки

Для прогнозування можуть бути корисними функції auto_ets та auto_arima пакету Julia StateSpaceModels
...
git-tree-sha1 = "d2526fc521343b0260c4ac4d175d998f6176fbc1"
