...
# ╔═╡ acf1482e-43c2-11ed-2080-35236b1ed6c4
using CommonMark

# ╔═╡ bf2fa3ef-3403-4328-ad49-8855adc5ab0a
using CSV

# ╔═╡ a29f4eee-cabd-48e2-b0a1-48b88ecabac8
using Statistics, DataFrames, Plots, Optim, Dates

# ╔═╡ 8d8dd65e-9f63-49fe-8f28-ce5189529049
cm"""
---
<div align="center">
...
# ╔═╡ f6f98409-e53c-4008-9d39-1113e724e4f7
cm"""
...
# ╔═╡ d6415970-1eb4-4cac-bdb3-47f431b52110
csv = CSV.File("data.csv"; select=["year", "total"], types=Dict(:year => Date));

# ╔═╡ f3ce9746-a90d-4f39-ae47-e8c0f6f8fcd1
df = DataFrame(csv);

# ╔═╡ 15e2c81f-dfef-4d70-be97-cd21bf692989
begin
    alphas = [0.2, 0.5, 0.8]
    results = []
	data = 0:10
...
# ╔═╡ 43b35c07-72dc-440b-91a5-525798aed6d8
begin
    scatter(results[1], label="α=0.2", title="Еволюція ваг з різними значеннями альфа")
...
# ╔═╡ 18ce485f-7c70-42ad-bd34-40914640ede2
...
# ╔═╡ 761462ae-15cc-420d-a26f-33382649fa4b
ts = df[!, "total"]

# ╔═╡ f3dc5198-8228-454c-ae55-3300191f51a0
pred_1 = SES_weight(0.2, ts[1], ts)

# ╔═╡ 7aed3bd0-38f1-4e4b-be44-b50c92b268c0
pred_2 = SES_weight(0.5, ts[1], ts)

# ╔═╡ a866bdad-ed6a-40fe-acbb-9a723356ba57
pred_3 = SES_weight(0.8, ts[1], ts)

# ╔═╡ 5c474f35-3748-47bd-add2-db398eeb6833
begin
...
# ╔═╡ 951ee10c-1d4c-43db-8c94-a9c2c314e562
...
# ╔═╡ e87c024a-5fd8-4f78-b949-405e8c84860c
begin
...
# ╔═╡ 27368c34-3603-403f-aebb-8218d0c415c5
function SES_loss_(params, time_serie=ts)
    return SES_weight_loss(params[1], params[2], time_serie)
end

# ╔═╡ c58e4dd7-c3f4-4976-8bd3-203a9adce557
begin
    lower = [0., ts[1]]
    upper = [1., ts[end]]
    initial_x = [0.6, mean(ts)]
    
    res = optimize(SES_loss_,lower, upper, initial_x)
end

# ╔═╡ b649d376-9d10-4478-b45f-c6cfe325388b
optim = Optim.minimizer(res)

# ╔═╡ 5c00800e-bbe8-4eb3-b580-c8b9083b10bf
...
# ╔═╡ db2598ab-b18b-42c1-a89e-8c6dba4103bd
forecast = SES_weight_forecast(optim[1], optim[2], ts, 3)

# ╔═╡ 29e65127-ba93-4b66-aeb5-9767335144f7
begin
    plot(1:length(ts), ts, label="Data", legend=:topleft)
    plot!(1:length(ts), forecast[1:length(ts)], label="Fitted")
    plot!(length(ts):length(forecast), forecast[length(ts):end], label="Forecast")
end

# ╔═╡ 1b277c2f-6c0e-4ddf-91b2-439c63c6ac6e
cm"""
### Висновки 
Запропонований алгоритм шукає оптимальне значення для α
"""
...
git-tree-sha1 = "6a563e756526f2f2f381e31d3125ca671ad6ebaa"
