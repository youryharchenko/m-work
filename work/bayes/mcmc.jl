...
# ╔═╡ 0f940a84-3774-11ee-14eb-a7f1cb7342e6
begin
using CairoMakie
using Distributions
using Random
...
# ╔═╡ dcf7dbd1-d26d-4174-b214-755280052ffd
using MCMCChains

# ╔═╡ b86f6b42-8525-48df-ad0d-984d0b29849d
using LinearAlgebra: eigvals, eigvecs

# ╔═╡ 5fcb0101-28bb-41e1-a90d-9df7610a512f
using PlutoUI

# ╔═╡ 01dc222b-9c32-48d9-8eb0-fdd3c843634c
using Turing

# ╔═╡ 9ed34a61-4879-44e6-a6f8-a3aa06d92370
using AlgebraOfGraphics

# ╔═╡ 99049c30-9193-492d-864f-8d2d3197595c
md"### https://storopoli.io/Bayesian-Julia/pages/05_MCMC/"

# ╔═╡ 898a66e8-26ad-421d-9898-d76f37b561ec
begin
const N = 100_000
const μ = [0, 0]
const Σ = [1 0.8; 0.8 1]
...
# ╔═╡ ceb8e383-8ac0-44bb-936a-13a9bc7625b7
mvnormal = MvNormal(μ, Σ)

# ╔═╡ 41ad33b1-f9e8-41d7-8e7d-a17801644e9d
let
	x = -3:0.01:3
	y = -3:0.01:3
...
# ╔═╡ a810166f-fa32-4e1b-9787-8b86f26af11e
let
	x = -3:0.01:3
	y = -3:0.01:3
	dens_mvnormal = [pdf(mvnormal, [i, j]) for i in x, j in y]
...
# ╔═╡ e34d5089-8a19-42e4-b63c-13908ea601c2
function metropolis(
    S::Int64,
    width::Float64,
    ρ::Float64;
...
# ╔═╡ 3e4dd4f8-1495-4dd2-9c20-ad0dc8f4b063
X_met = let
	S = 10_000
	width = 2.75
	ρ = 0.8

	metropolis(S, width, ρ);
end

# ╔═╡ eaac13c2-c929-40a3-af60-3adc4b21a4b4

chain_met = Chains(X_met, [:X, :Y])

# ╔═╡ 9da94f17-5a92-4025-b88d-a0ee0936b37c
summarystats(chain_met)

# ╔═╡ e958dc2b-c7aa-43b2-b283-672e551d18d3
...
# ╔═╡ 32ce0798-932b-4383-879c-d50d1e5d8817
...
# ╔═╡ 5a1d498f-2f2a-40c3-9d29-19b0114f62db
let
	f, ax, l = lines(
	...
	axislegend(ax)
	record(f, joinpath(".", "met_anim.gif"); framerate=5) do frame
    	for i in 1:100
...
# ╔═╡ d2c42048-8efb-44dd-86c8-4b6338c47977
LocalResource(joinpath(".", "met_anim.gif"))

# ╔═╡ 05e84588-1116-4abf-889c-0cd25ca26f40
let
	warmup = 1_000
...
# ╔═╡ e9472df6-5b63-493f-89d9-0ecbf25158cc
let
	warmup = 1_000
...
# ╔═╡ 20fc2094-954a-4989-aec4-a54ddd6283c1
Dirichlet(6, 1)

# ╔═╡ 3092a78b-b007-4e7d-98a2-6ce8cdda32cc
...
# ╔═╡ 02577d3b-dfec-4e6e-b162-4eaf83dfefc4
data_dice = rand(DiscreteUniform(1, 6), 1_000)

# ╔═╡ 03a740ea-c824-4020-bbc8-60e1437afdfe
model = dice_throw(data_dice)

# ╔═╡ c271b378-1e18-48d3-ba2d-b7ff57033eea
chain = sample(model, NUTS(), 1_000)

# ╔═╡ 5fcc3f87-f8dd-4b35-bfaf-eec5b2b36dd1
summarystats(chain)

# ╔═╡ 838931cf-cd59-453b-8114-51300fc97570
let
...
# ╔═╡ 316feada-bf87-41e0-8934-ad31ad666af0
bad_chain = sample(model, NUTS(0.3), 500)

# ╔═╡ 821d4804-4143-4ce6-9110-9df669b5e333
let
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "f0a672a49cfe043d926023f9d35360e08af781ec"
