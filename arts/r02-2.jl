...
m = let 
	...
	nc = ncol(df)
	ni = Int(nr / l)
	mx = Matrix{Float64}(undef, ni, nc-2)
	nmonth = Vector{Float64}(undef, ni)
	npart = Vector{Float64}(undef, ni)
	buf = Matrix{Float64}(undef, l, nc-2)
	...
	for i in 1:nr
		...
		if k == l
			npart[ii] = df[i, :HOUR] / l
			nmonth[ii] = month(ts[i])
			mx[ii, :] = mean(buf, dims=1)
			k = 1
			ii += 1
		else
			k += 1
...
# ╔═╡ 66ed194a-78cc-458a-a076-5f32fdd9fa31
